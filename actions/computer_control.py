...
def _safe_screenshot_path(requested: str | None) -> Path:
    """Resolve a user-friendly path to a safe screenshot save location."""
    from datetime import datetime

    # No path given → timestamped file on the desktop
    if not requested:
        ts = datetime.now().strftime("Screenshot_%Y-%m-%d_%H-%M-%S.png")
        return Path.home() / "Desktop" / ts

    try:
        req = requested.strip().strip('"').strip("'").replace("\\", "/")

        # If the entire request is just a known folder name, save a timestamped file there
        folder_key = req.rstrip("/").lower()
        if folder_key in _KNOWN_FOLDERS:
            ts = datetime.now().strftime("Screenshot_%Y-%m-%d_%H-%M-%S.png")
            return _KNOWN_FOLDERS[folder_key] / ts

        # Otherwise, treat it as a path
        p = Path(req)

        # If it's just a filename with extension, save to desktop
        if p.parent == Path('.'):
            return Path.home() / "Desktop" / p.name

        # If the first part is a known folder, substitute the real path
        parts = p.parts
        if parts[0].lower() in _KNOWN_FOLDERS:
            base = _KNOWN_FOLDERS[parts[0].lower()]
            p = base.joinpath(*parts[1:])
        else:
            # Assume it's an absolute path or relative to CWD
            p = p.expanduser().resolve()

        # Safety: ensure it stays somewhere under the home directory
...
def _screenshot(save_path: str | None = None) -> str:
    """Take a screenshot and save it. Uses mss for speed and reliability."""
    try:
        import mss
        import mss.tools
