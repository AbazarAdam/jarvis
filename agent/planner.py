...
def _fallback_plan(goal: str) -> dict:
    ...
    # --- Direct file conversion / reading (no web search) ---
    if any(word in goal_lower for word in ["convert", "to pdf", "to_pdf", "read", "resize", "rename"]):
        # Try to extract a filename from the goal
        filename = "file.txt"
        # Look for a filename pattern (something .txt, .docx, .png, etc.)
        import re
        fname_match = re.search(r'[\w\s-]+\.[a-z]{2,5}', goal)
        if fname_match:
            filename = fname_match.group().strip()

        steps = []

        # If "read" is in goal, just use file_controller to read
        if "read" in goal_lower:
            steps.append({
                "step": 1,
                "tool": "file_controller",
                "description": f"Read the file {filename} from the desktop",
                "parameters": {"action": "read", "path": "desktop", "name": filename}
            })
    ...
    # Detect if the user wants to save to a file
    filename = None
        ...
        if keyword in goal_lower:
            ...
            if len(parts) > 1:
                ...
                if "." in candidate or len(candidate) > 2:
                    filename = candidate if "." in candidate else candidate + ".txt"
                    break
    if not filename:
        words = goal.split()
        if len(words) > 3:
            filename = "_".join(words[:3]) + ".txt"
        else:
            filename = "research_output.txt"
    steps.append({
        "step": 2,
        "tool": "file_controller",
        "description": f"Save the research summary to {filename} on the desktop",
        "parameters": {
            "action": "write",
            "path": "desktop",
            "name": filename,
