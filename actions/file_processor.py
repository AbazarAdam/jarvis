...
def _ai_generate_text(prompt: str, max_tokens: int = 1500, temperature: float = 0.3) -> str:
    """Unified text generation through ModelRouter with provider fallback."""
    ...
    if not response.get("success"):
        raise RuntimeError(response.get("error") or "All AI providers failed")
...
def _convert_word_to_pdf(path: Path) -> str:
    """Convert a .docx file to PDF using Word (Windows) or LibreOffice (fallback)."""
    try:
        import docx2pdf
        output = path.with_suffix(".pdf")
        docx2pdf.convert(str(path), str(output))
        return f"Converted {path.name} → {output}"
    except Exception as e:
        return f"Word→PDF conversion failed: {e}"
...
def _process_pdf(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    def _extract_pdf_text(max_chars=50000000000) -> str:
        text = ""
        try:
            import pdfplumber
    ...
    if action in ("summarize", "extract_text", "translate_hint", "analyze", "reformat"):
        ...
        try:
            result = _ai_generate_text(prompt_map.get(action, f"Analyze:\n\n{text}"))
...
def _convert_text_to_pdf(txt_path: Path) -> str:
    """Convert a .txt file to PDF using fpdf2 (no external dependencies)."""
    try:
        ...
        for line in content.splitlines():
            # fpdf2 can handle most Unicode; encode/decode to be safe
            pdf.multi_cell(0, 5, txt=line.encode("latin-1", "replace").decode("latin-1"))
        output_pdf = txt_path.with_suffix(".pdf")
        pdf.output(str(output_pdf))
        return f"Converted {txt_path.name} → {output_pdf}"
    except Exception as e:
        return f"Text→PDF conversion failed: {e}"
...
def _process_text_doc(path: Path, file_type: str, action: str,
    ...
    # Word → PDF conversion
    if file_type == "docx" and action == "to_pdf":
        return _convert_word_to_pdf(path)

    # Plain text → PDF conversion (NEW)
    if file_type == "text" and action == "to_pdf":
        return _convert_text_to_pdf(path)
    ...
    try:
        result = _ai_generate_text(prompt_map[action])
        if len(result) > 600 and params.get("save", True):
            out = _output_path(path, action, ".txt")
            out.write_text(result, encoding="utf-8")
            return f"{result[:400]}...\n\nFull result saved: {out.name}"
...
def _process_data(path: Path, file_type: str, action: str,
    ...
    if action == "analyze":
        ...
        try:
            return _ai_generate_text(prompt)
        except Exception as e:
            return f"AI analysis failed: {e}"
...
def _process_code(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    try:
        result = _ai_generate_text(prompt)
...
def file_processor(parameters: dict, player=None, speak=None) -> str:
    ...
    # Expand shortcuts like "desktop", "downloads", etc.
    shortcuts = {
    ...
    # Check if the string starts with a known shortcut (with or without trailing /)
    lower_path = file_path_str.strip().replace("\\", "/").lower()
    for key, base in shortcuts.items():
        if lower_path == key or lower_path.startswith(key + "/"):
            # Replace shortcut with real path
            rest = file_path_str.strip()[len(key):].lstrip("/").lstrip("\\")
            file_path_str = str(base / rest) if rest else str(base)
            break
