...
def set_volume_windows(level: int) -> str:
...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        return set_volume_windows(value)
    elif _OS == "Darwin":
...
def set_brightness(value: int):
    ...
    if _OS == "Windows":
        if _SBC:
            try:
                ...
                print(f"[Settings] screen_brightness_control failed: {e}")
    ...
    if _OS == "Darwin":
        try:
            subprocess.run(["brightness", str(value / 100.0)], capture_output=True)
            return f"Brightness set to {value}%"
        except Exception as e:
            return f"Could not set brightness: {e}"
    ...
    return "Brightness control is not available on this device."
...
def control_window_by_title(title: str, action: str) -> str:
    """Minimize, maximize, or close a window by its title (partial match)."""
    ...
    try:
        ...
        return f"Failed to {action} '{win.title}': {e}"

def minimize_window(window_title: str = None):
    if window_title:
        return control_window_by_title(window_title, "minimize")
    # Fallback: minimize active window
    if _OS == "Darwin": pyautogui.hotkey("command", "m")
    else:               pyautogui.hotkey("win", "down")

def maximize_window(window_title: str = None):
    if window_title:
        return control_window_by_title(window_title, "maximize")
...
def close_window(window_title: str = None):
    if window_title:
        return control_window_by_title(window_title, "close")
    # Fallback: close active window
    if _OS == "Darwin": pyautogui.hotkey("command", "w")
    else:               pyautogui.hotkey("ctrl", "w")

def close_app(window_title: str = None):
    """Close an application window by title, or use Alt+F4 on active."""
    if window_title:
        return control_window_by_title(window_title, "close")
    # Fallback: close active application
    if _OS == "Darwin":
...
def take_screenshot():
    """Take a screenshot using mss and save it to the Desktop with a timestamp."""
    ...
    try:
        # Generate a timestamped filename on the Desktop
        ts = datetime.now().strftime("Screenshot_%Y-%m-%d_%H-%M-%S.png")
        filepath = Path.home() / "Desktop" / ts
...
- brightness_set only works on laptops or devices with a controllable built-in display.
...
def computer_settings(
    ...
    if action == "volume_set":
        try:
            target_value = int(value or 50)
            return volume_set(target_value) or f"Volume set to {target_value}%"
        except Exception as e:
            return f"Could not set volume: {e}"
    ...
    if action in ("brightness_set", "set_brightness"):
        try:
            target_value = int(value or 50)
            return set_brightness(target_value)
        except Exception as e:
            return f"Could not set brightness: {e}"
    ...
    # Window control (by title, then fallback to hotkey)
    if action in ("minimize", "maximize", "close_window", "close_app"):
        ...
        if window_title:
            if action == "minimize":
                ...
                return close_app(window_title)
        # No title – use hotkey (fallback)
    ...
    # All other actions (screenshot, lock, etc.)
    func = ACTION_MAP.get(action)
    if not func:
